...
public class MainActivity extends AppCompatActivity {
    ...
    private void signOut(){if(!requireOnline())return;Runnable finish=()->{PushTokenRegistration.cancelPending(this);pushRegistrationStarted=false;api.clearToken();sessions.clear();snapshots.clear();data=null;showLogin();};api.post("/auth/logout",new JSONObject(),new FundsApi.Callback(){public void success(JSONObject ignored){finish.run();}public void error(String ignored){finish.run();}});}
    ...
    private void showLogin(){
        ...
        EditText id=input("System-issued user ID");id.setText(sessions.exists()?sessions.credentialId():"");id.setInputType(InputType.TYPE_CLASS_TEXT|InputType.TYPE_TEXT_FLAG_CAP_CHARACTERS);content.addView(id,new LinearLayout.LayoutParams(-1,NativeUi.dp(this,54)));
    ...
    private void showRequiredPasswordChange(){
        ...
        ScrollView scroll=new ScrollView(this);scroll.setFillViewport(true);LinearLayout page=new LinearLayout(this);page.setOrientation(LinearLayout.VERTICAL);page.setPadding(NativeUi.dp(this,24),NativeUi.dp(this,34),NativeUi.dp(this,24),NativeUi.dp(this,34));scroll.addView(page);
        TextView mark=NativeUi.text(this,"F",23,Color.WHITE,true);mark.setGravity(Gravity.CENTER);mark.setBackground(NativeUi.shape(this,NativeUi.INK,16));page.addView(mark,new LinearLayout.LayoutParams(NativeUi.dp(this,56),NativeUi.dp(this,56)));
        page.addView(NativeUi.text(this,"Create your private password",25,NativeUi.INK,true),NativeUi.margins(this,new LinearLayout.LayoutParams(-1,NativeUi.dp(this,72)),0,20,0,0));
        TextView explanation=NativeUi.text(this,"Replace the temporary password issued by your administrator before entering FUNDSHIP.",13,NativeUi.MUTED,false);explanation.setLineSpacing(0,1.15f);page.addView(explanation,new LinearLayout.LayoutParams(-1,NativeUi.dp(this,58)));
        EditText current=input("Initial password");current.setInputType(InputType.TYPE_CLASS_TEXT|InputType.TYPE_TEXT_VARIATION_PASSWORD);page.addView(NativeUi.labeled(this,"INITIAL PASSWORD",current,"Enter the password given to you."),NativeUi.margins(this,new LinearLayout.LayoutParams(-1,-2),0,18,0,0));
        EditText next=input("New password");next.setInputType(InputType.TYPE_CLASS_TEXT|InputType.TYPE_TEXT_VARIATION_PASSWORD);page.addView(NativeUi.labeled(this,"NEW PASSWORD",next,"Use at least 8 characters."),NativeUi.margins(this,new LinearLayout.LayoutParams(-1,-2),0,12,0,0));
        EditText confirm=input("Confirm new password");confirm.setInputType(InputType.TYPE_CLASS_TEXT|InputType.TYPE_TEXT_VARIATION_PASSWORD);page.addView(NativeUi.labeled(this,"CONFIRM PASSWORD",confirm,"Enter the same new password again."),NativeUi.margins(this,new LinearLayout.LayoutParams(-1,-2),0,12,0,0));
    ...
    private void showRequiredMpinSetup(){
        ...
        ScrollView scroll=new ScrollView(this);scroll.setFillViewport(true);LinearLayout page=new LinearLayout(this);page.setOrientation(LinearLayout.VERTICAL);page.setPadding(NativeUi.dp(this,24),NativeUi.dp(this,34),NativeUi.dp(this,24),NativeUi.dp(this,34));scroll.addView(page);
        TextView mark=NativeUi.text(this,"F",23,Color.WHITE,true);mark.setGravity(Gravity.CENTER);mark.setBackground(NativeUi.shape(this,NativeUi.INK,16));page.addView(mark,new LinearLayout.LayoutParams(NativeUi.dp(this,56),NativeUi.dp(this,56)));
        page.addView(NativeUi.text(this,"Create your 4-digit MPIN",25,NativeUi.INK,true),NativeUi.margins(this,new LinearLayout.LayoutParams(-1,NativeUi.dp(this,72)),0,20,0,0));
        TextView explanation=NativeUi.text(this,"Your MPIN protects payment and request approvals when fingerprint is unavailable.",13,NativeUi.MUTED,false);explanation.setLineSpacing(0,1.15f);page.addView(explanation,new LinearLayout.LayoutParams(-1,NativeUi.dp(this,58)));
        EditText mpin=input("New MPIN");mpin.setInputType(InputType.TYPE_CLASS_NUMBER|InputType.TYPE_NUMBER_VARIATION_PASSWORD);page.addView(NativeUi.labeled(this,"NEW MPIN",mpin,"Choose exactly 4 digits."),NativeUi.margins(this,new LinearLayout.LayoutParams(-1,-2),0,18,0,0));
        EditText confirm=input("Confirm MPIN");confirm.setInputType(InputType.TYPE_CLASS_NUMBER|InputType.TYPE_NUMBER_VARIATION_PASSWORD);page.addView(NativeUi.labeled(this,"CONFIRM MPIN",confirm,"Enter the same 4 digits again."),NativeUi.margins(this,new LinearLayout.LayoutParams(-1,-2),0,12,0,0));
    ...
    private void showNotifications(){
        ...
        JSONArray items=inboxNotifications();LinearLayout content=dialogList();FundshipSheet[] sheetRef=new FundshipSheet[1];if(items.length()==0){LinearLayout empty=NativeUi.sectionCard(this);empty.setGravity(Gravity.CENTER);TextView emptyTitle=NativeUi.text(this,"You’re all caught up",16,NativeUi.INK,true);emptyTitle.setGravity(Gravity.CENTER);empty.addView(emptyTitle,new LinearLayout.LayoutParams(-1,NativeUi.dp(this,38)));TextView emptyCopy=NativeUi.text(this,"Group and connection updates will appear here.",12,NativeUi.MUTED,false);emptyCopy.setGravity(Gravity.CENTER);emptyCopy.setTextAlignment(View.TEXT_ALIGNMENT_CENTER);empty.addView(emptyCopy,new LinearLayout.LayoutParams(-1,NativeUi.dp(this,54)));content.addView(empty,new LinearLayout.LayoutParams(-1,NativeUi.dp(this,122)));}
            ...
            if("connection_request".equals(item.optString("type"))){JSONObject request=findById(data.optJSONArray("connectionRequests"),item.optString("entityId"));if(request!=null&&!request.optBoolean("outgoing")){TextView decline=NativeUi.button(this,"Decline",NativeUi.RED,NativeUi.RED_SOFT,9),accept=NativeUi.button(this,"Accept",Color.WHITE,NativeUi.GREEN,9);row.addView(decline,new LinearLayout.LayoutParams(NativeUi.dp(this,70),NativeUi.dp(this,36)));row.addView(accept,NativeUi.margins(this,new LinearLayout.LayoutParams(NativeUi.dp(this,66),NativeUi.dp(this,36)),5,0,0,0));decline.setOnClickListener(v->{sheetRef[0].dismiss();respondConnection(request.optString("id"),false);});accept.setOnClickListener(v->{sheetRef[0].dismiss();respondConnection(request.optString("id"),true);});continue;}}
            if("group_invite".equals(item.optString("type"))){JSONObject invite=findById(data.optJSONArray("groupInvites"),item.optString("entityId"));if(invite!=null){TextView decline=NativeUi.button(this,"Decline",NativeUi.RED,NativeUi.RED_SOFT,9),accept=NativeUi.button(this,"Accept",Color.WHITE,NativeUi.GREEN,9);row.addView(decline,new LinearLayout.LayoutParams(NativeUi.dp(this,70),NativeUi.dp(this,36)));row.addView(accept,NativeUi.margins(this,new LinearLayout.LayoutParams(NativeUi.dp(this,66),NativeUi.dp(this,36)),5,0,0,0));decline.setOnClickListener(v->{sheetRef[0].dismiss();respondInvite(invite.optString("id"),false);});accept.setOnClickListener(v->{sheetRef[0].dismiss();respondInvite(invite.optString("id"),true);});continue;}}
    ...
    private JSONArray inboxNotifications(){JSONArray result=new JSONArray();if(data==null)return result;for(JSONObject item:NativeUi.objects(data.optJSONArray("notifications"))){String type=item.optString("type");if("payment_request".equals(type)||type.startsWith("poll_")||"event_due".equals(type))continue;result.put(item);}return result;}
    ...
    void showTransactionHistory(JSONObject person){
        ...
        FundshipSheet.show(this,"Ledger","History with "+NativeUi.displayName(person.optString("name")),count+" transaction"+(count==1?"":"s")+" · verified value "+NativeUi.money(verifiedValue),content,null,88,null);
    }
    ...
    private void showProfile(){
        ...
        TextView connectionHelp=NativeUi.text(this,"Enter a system-issued user ID to send a connection request.",10,NativeUi.MUTED,false);content.addView(connectionHelp,NativeUi.margins(this,new LinearLayout.LayoutParams(-1,NativeUi.dp(this,30)),2,5,0,0));
        ...
        JSONArray connections=data.optJSONArray("connections");if(connections==null||connections.length()==0){LinearLayout empty=NativeUi.sectionCard(this);TextView emptyText=NativeUi.text(this,"No connections yet. Enter a user ID above to connect.",12,NativeUi.MUTED,false);emptyText.setGravity(Gravity.CENTER);empty.addView(emptyText,new LinearLayout.LayoutParams(-1,NativeUi.dp(this,62)));content.addView(empty,new LinearLayout.LayoutParams(-1,NativeUi.dp(this,88)));}else for(JSONObject person:NativeUi.objects(connections)){LinearLayout row=simpleRow("✓",person.optString("name"),person.optString("credentialId")+" · Tap to view transaction history");TextView next=NativeUi.text(this,"›",22,NativeUi.MUTED,false);next.setGravity(Gravity.CENTER);row.addView(next,new LinearLayout.LayoutParams(NativeUi.dp(this,28),NativeUi.dp(this,40)));row.setContentDescription("View transaction history with "+person.optString("name"));row.setOnClickListener(v->{sheetRef[0].dismiss();showTransactionHistory(person);});content.addView(row,NativeUi.margins(this,new LinearLayout.LayoutParams(-1,-2),0,0,0,8));}
    ...
    private void deliverNotifications(){
        PaymentNotificationManager.reconcile(this,data.optJSONObject("payments").optJSONArray("incoming"));
        for(JSONObject item:NativeUi.objects(data.optJSONArray("notifications"))){if(item.optBoolean("nativeDelivered"))continue;boolean shown=false;String type=item.optString("type");if(type.equals("payment_request")){JSONObject payment=findById(data.optJSONObject("payments").optJSONArray("incoming"),item.optString("entityId"));if(payment!=null)shown=PaymentNotificationManager.wasDelivered(this,payment.optString("id"))||PaymentNotificationManager.showIncoming(this,payment.optString("id"),payment.optString("payeeName"),(int)Math.round(payment.optDouble("amount")),payment.optString("purpose"));}else if(type.equals("poll_open")){JSONObject[] owner=findPoll(item.optString("entityId"));if(owner!=null&&(owner[1].isNull("myVote")||owner[1].optString("myVote").isEmpty())){PollPayload payload=payload(owner[0],owner[1]);shown=PollNotificationManager.wasDelivered(this,payload.pollId)||PollNotificationManager.show(this,payload);}}
