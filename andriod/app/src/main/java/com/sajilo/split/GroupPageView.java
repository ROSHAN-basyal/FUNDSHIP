...
final class GroupPageView extends FrameLayout {
    ...
    GroupPageView(MainActivity context,MainActivity host,JSONObject group){super(context);this.host=host;this.group=group;this.data=host.data();build();}
    ...
    private void vote(JSONObject poll,String choice){JSONObject body=new JSONObject();try{body.put("choice",choice);}catch(Exception ignored){}host.api().post("/polls/"+poll.optString("id")+"/vote",body,host.callbackRefresh("Vote recorded"));}
