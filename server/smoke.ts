...
try{
  ...
  const u7=await login('BT-007','initial88');let onboarding=await api('/bootstrap',u7);assert(onboarding.user.mustChangePassword&&!onboarding.user.hasMpin,'New account onboarding flags missing');
  ...
  await api('/auth/change-password',u7,'POST',{oldPassword:'initial88',newPassword:'private-beta-88'});onboarding=await api('/bootstrap',u7);assert(!onboarding.user.mustChangePassword&&!onboarding.user.hasMpin,'Password onboarding did not advance to MPIN setup');
  ...
  await api('/auth/set-mpin',u7,'POST',{newMpin:'7788'});onboarding=await api('/bootstrap',u7);assert(onboarding.user.hasMpin,'Initial MPIN was not saved');const logoutToken=await login('BT-007','private-beta-88');await api('/auth/logout',logoutToken,'POST');const loggedOut=await fetch(`${root}/bootstrap`,{headers:{Authorization:`Bearer ${logoutToken}`}});assert(loggedOut.status===401,'Server-side logout did not revoke the session');
  ...
  await api('/groups/g1/messages',u1,'POST',{body:'live sync smoke'});const liveMessages=await api('/groups/g1/messages',u2);assert(liveMessages.messages.some((item:any)=>item.body==='live sync smoke'),'Live chat sync endpoint did not return the new message');
