...
The public Android download page is served at
`https://fundship-five.vercel.app/download`. Its versioned APK is delivered
from the public Supabase Storage bucket `app-releases`; see
[`docs/android-release-distribution.md`](docs/android-release-distribution.md)
for signing, verification, upload, and update instructions.

1. Create a Supabase project and run [`supabase/migrations/202607230001_initial_schema.sql`](supabase/migrations/202607230001_initial_schema.sql) in its SQL editor. Existing deployments should then apply later migration files in filename order, including [`supabase/migrations/202607240001_group_management.sql`](supabase/migrations/202607240001_group_management.sql) and [`supabase/migrations/202607250001_payment_request_lifecycle.sql`](supabase/migrations/202607250001_payment_request_lifecycle.sql). Run [`supabase/seed.sql`](supabase/seed.sql) only if the demo accounts are wanted.
