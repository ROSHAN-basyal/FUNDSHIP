...
import { applyPollVote, cachedBootstrap, getBootstrap, getSync, mutate, rememberBootstrap, request, session } from './lib/api';
...
function AppShell({data,onData}:{data:Bootstrap;onData:(d:Bootstrap)=>void}){
  ...
  useEffect(()=>{const sync=async()=>{if(document.visibilityState!=='visible'||syncing.current)return;syncing.current=true;try{const result=await getSync(data.revision||0);if(result.changed)onData(rememberBootstrap(result.snapshot))}catch{/* retain the last usable screen while temporarily offline */}finally{syncing.current=false}};const timer=window.setInterval(()=>void sync(),6000);const visible=()=>{if(document.visibilityState==='visible')void sync()};document.addEventListener('visibilitychange',visible);return()=>{clearInterval(timer);document.removeEventListener('visibilitychange',visible)}},[data.revision,onData]);
  ...
  return <div className="app-shell" onTouchStart={e=>touchX.current=e.changedTouches[0].clientX} onTouchEnd={e=>{const d=e.changedTouches[0].clientX-touchX.current;if(Math.abs(d)>85)movePage(d<0?1:-1)}}>
    <header className="app-header">
      ...
      <div className="header-tools"><button className="notification-btn" onClick={()=>setNotificationOpen(true)}><Bell size={19}/>{data.notifications.some(item=>!item.read&&item.type!=='payment_request'&&!item.type.startsWith('poll_')&&item.type!=='event_due')&&<i/>}</button><button className="profile-button" onClick={()=>setProfileOpen(true)}><Avatar name={data.user.name} color={data.user.avatarColor} size="sm"/><span><strong>{data.user.name.split(' ')[0]}</strong><small>{data.user.credentialId}</small></span><Settings2 size={15}/></button><button className="logout-btn" onClick={()=>void logout()} title="Sign out"><LogOut size={18}/></button></div>
    </header>
    <main className="app-main">
      ...
      <div key={page} className={`page-transition slide-${slideDirection}`}>{page==='home'?<HomePage data={data} onData={onData} notify={notify}/>:currentGroup?<GroupPage data={data} group={currentGroup} userId={data.user.id} onData={onData} notify={notify}/>:null}</div>
    </main>
